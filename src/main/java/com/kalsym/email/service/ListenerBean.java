            ...
            map.forEach((RequestMappingInfo requestMappingInfo, HandlerMethod handlerMethod) -> {
                try {
                    if (!handlerMethod.getMethod().getName().equalsIgnoreCase("error")
                        ...
                        Logger.application.info(Logger.pattern, EmailServiceApplication.VERSION, "", "name: " + requestMappingInfo.getName());
                        Logger.application.info(Logger.pattern, EmailServiceApplication.VERSION, "", "method: " + handlerMethod.getMethod().getName());
                        Logger.application.info(Logger.pattern, EmailServiceApplication.VERSION, "", "description: " + requestMappingInfo.toString());
                        ...
                        authority.setServiceId("email-service");

                        if (null != authority.getId()) {
                            authorities.add(authority);
                        }
                        Logger.application.info(Logger.pattern, EmailServiceApplication.VERSION, "", "inserted authority", "");

                    }

                } catch (Exception e) {
                    Logger.application.warn(Logger.pattern, EmailServiceApplication.VERSION, "error inserting authority", e.getMessage());
                }

            });

            ResponseEntity<String> response = restTemplate.postForEntity(userServiceUrl, authorities, String.class);
            Logger.application.info(Logger.pattern, EmailServiceApplication.VERSION, "response: " + response);
